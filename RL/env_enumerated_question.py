...
class EnumeratedRecommendEnv(object):
    ...
    def reset(self, embed=None):
        ...
        i_belong = [item for item in range(self.item_length) if
                    len(set(self.kg.G['item'][item]['belong_to_large']) - f_item) == 0 and item != self.target_item]

        self.cand_items = set(set(list(range(self.item_length))) - set(i_belong))
        ...
        self.cand_len = [self.feature_length >> d & 1 for d in range(self.cand_item_num)][::-1]
        ...
        return self._get_scpr_state(), self._get_state(), cand_feature, cand_item, ac_label, self._get_action_space()
        ...
        if self.random_sample_item:
            ...
            if len(self.cand_items) > 10:
                for item in self.cand_items:
                    ...
                    if len(self.kg.G['item'][item]['belong_to_large']) == len(self.user_acc_feature) + 1:
                        fea = list(set(self.kg.G['item'][item]['belong_to_large']) - set(self.user_acc_feature))[0]
                        ...
                        item_o.append(item)
                # item_fld=[item for item in self.cand_items if len(self.kg.G['item'][item]['belong_to'])==len(self.user_acc_feature)+1]
                cand_item = self._map_to_all_id(item_fld[:self.cand_item_num], 'item')
            ...
            if len(cand_item)<self.cand_item_num and len(cand_item)>0:
                    cand_item+=self._map_to_all_id(item_o[:self.cand_item_num-len(cand_item)],'item')
                
            #cand_item = self._map_to_all_id(self.cand_items[:self.cand_item_num],'item')
        #cand = cand_feature + cand_item
        return cand_feature, cand_item, (self.target_item + self.user_length) in cand_item
    ...
    def step(self, action, sorted_actions, embed=None, is_sample=False):
        ...
        if self.cur_conver_step == self.max_turn:
            ...
            asked_feature = self.kg.G['feature'][self._map_to_old_id(action)]['link_to_feature'][0]#self.reachable_feature[0]
        ...
        self.cur_conver_step += 1
        cand_feature, cand_item, ac_label= self._get_cand()
        return self._get_scpr_state(),self._get_state(), cand_feature, cand_item, ac_label, self._get_action_space(), reward, done
    ...
    def _update_cand_items(self, asked_feature, acc_rej):
        ...
        if acc_rej:    # accept feature
            ...
            if isinstance(asked_feature, list):
                ...
                for fea in asked_feature:
                    feature_small_ids = self.kg.G['large_feature'][fea]['link_to_feature']
                    for small_id in feature_small_ids:
        ...
        self.cand_len = [len(self.cand_items) >> d & 1 for d in range(self.cand_item_num)][::-1]
