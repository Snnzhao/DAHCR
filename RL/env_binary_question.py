...
class BinaryRecommendEnv(object):
    ...
    def reset(self, embed=None):
        ...
        i_belong = [item for item in range(self.item_length) if
                    len(set(self.kg.G['item'][item]['belong_to']) - f_item) == 0 and item != self.target_item]
        ...
        if self.random_sample_item:
            ...
            if len(self.cand_items)>20:
                for item in self.cand_items:
                    if len(self.kg.G['item'][item]['belong_to'])==len(self.user_acc_feature)+1:
                        fea=list(set(self.kg.G['item'][item]['belong_to'])-set(self.user_acc_feature))[0]
    ...
    def step(self, action, sorted_actions, embed=None, is_sample=False):
        ...
        #     #asked_feature = self._map_to_old_id(self.reachable_feature[0])#self._map_to_old_id(action)
        #     asked_feature = self._map_to_old_id(action)
        #     print('-->action: hard ask features {}, max entropy feature {}'.format(asked_feature, self.reachable_feature[:self.cand_num]))
