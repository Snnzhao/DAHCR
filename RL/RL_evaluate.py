...
def dqn_evaluate(args, kg, dataset, agent, filename, i_episode):
    ...
    for user_num in tqdm(range(user_size)):  #user_size
        ...
        if not args.fix_emb:
            scpr,state, cand_feature, cand_item, ac_label, action_space = test_env.reset(agent.gcn_net.embedding.weight.data.cpu().detach().numpy())  # Reset environment and record the starting state
        else:
        ...
        for t in count():  # user  dialog
            if t == 14:
                is_last_turn = True
            action, sorted_actions, is_sample = agent.select_action(scpr,state,  cand_feature, cand_item, action_space, is_test=True, is_last_turn=is_last_turn, turn=t)
            next_scpr,next_state,  next_cand_feature, next_cand_item,nac_label, action_space, reward, done = test_env.step(action.item(), sorted_actions)
            epi_reward += reward
